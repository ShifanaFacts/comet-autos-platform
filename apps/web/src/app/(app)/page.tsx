...
export default async function DashboardPage() {
  ...
  const attention = [
    data.waitingForApproval > 0
      ? {
          key: 'approval',
          label: `${data.waitingForApproval} quotation${data.waitingForApproval === 1 ? '' : 's'} waiting for approval`,
    ...
    data.onHold > 0
      ? {
          key: 'hold',
          label: `${data.onHold} vehicle${data.onHold === 1 ? '' : 's'} on hold`,
    ...
    data.lowStockParts.length > 0
      ? {
          key: 'stock',
          label: `${data.lowStockParts.length} part${data.lowStockParts.length === 1 ? '' : 's'} low on stock`,
          href: '/job-cards',
        }
      : null,
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Section
        ...
        description={`${data.todaysAppointments} appointment${data.todaysAppointments === 1 ? '' : 's'} today · ${data.vehiclesCurrentlyIn} vehicle${data.vehiclesCurrentlyIn === 1 ? '' : 's'} currently inside`}
      >
        <WorkshopFlowRow stages={data.workflowStages} />
      </Section>

      <div className="grid gap-6 md:grid-cols-2">
        <Section title="Finance snapshot">
          <div className="grid grid-cols-3 gap-3">
            <StatTile label="Today's sales" value={<MoneyDisplay amount={data.todaysSales} />} />
            <StatTile label="Collections" value={<MoneyDisplay amount={data.todaysCollections} />} />
            <StatTile label="Outstanding" value={<MoneyDisplay amount={data.customerOutstanding} />} />
          </div>
        </Section>
