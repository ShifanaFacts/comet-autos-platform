...
export function EstimateBuilder({
  ...
  function payload() {
    return {
      ...
      items: lines
        ...
        .map(({ itemType, description, quantity, unitPrice, taxRate }) => ({ itemType, description, quantity, unitPrice, taxRate })),
    };
  }
  ...
  function send() {
    ...
    startTransition(async () => {
      ...
      setPending(null);
      if (!result.ok || !result.data) return setError(result.error ?? 'Could not send the estimate.');
  ...
  if (link) {
    return <CustomerLinkPanel link={link} customerName={customerName} onDone={() => router.refresh()} />;
  }
    ...
    <div className="flex flex-col gap-8">
      {recommendation ? (
        <div className="rounded-xl border border-border bg-muted/40 px-4 py-4 sm:px-6">
          <p className="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Recommended work (from diagnosis)</p>
          <p className="mt-2 text-sm whitespace-pre-wrap">{recommendation}</p>
        </div>
      ) : null}

      {[
        { type: 'LABOUR' as const, title: 'Labour', icon: Wrench, rows: labour, qtyLabel: 'Hours', priceLabel: 'Rate (AED)' },
        { type: 'PART' as const, title: 'Parts', icon: Package, rows: parts, qtyLabel: 'Qty', priceLabel: 'Unit price (AED)' },
      ].map((group) => (
                    ...
                    <tr key={line.key} className="align-top">
                      <td className="px-4 py-3 pl-6">
                        <Input
                          ...
                          placeholder={group.type === 'LABOUR' ? 'e.g. Replace AC compressor clutch' : 'e.g. Front brake pad set'}
                        />
                      </td>
                      ...
                      >
                        {amounts ? formatMoney(amounts.lineTotal) : line.unitPrice === '' ? '—' : 'Check'}
                      </td>
      ...
      <div className="grid gap-8 border-t border-border pt-8 md:grid-cols-2">
        <Field label="Quotation valid until" htmlFor="validUntil" required hint="The customer's link stops working after this date.">
          <Input
            id="validUntil"
            type="date"
            min={minValidUntil}
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border pt-6">
        {incomplete ? (
          <p className="mr-auto text-sm text-muted-foreground">Every line needs a description and a valid amount before sending.</p>
        ) : null}
