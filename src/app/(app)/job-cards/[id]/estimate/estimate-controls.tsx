...
import { createEstimateAction, recordDecisionAction, reissueLinkAction, reviseEstimateAction } from '../actions';
...
export function CreateEstimateButton({ jobCardId }: { jobCardId: string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <Button size="lg" className="self-start" disabled={isPending} onClick={() => run(() => createEstimateAction(jobCardId), 'Estimate created')}>
  ...
  return (
    <div className="flex flex-col gap-2">
      <ConfirmAction
        ...
        onConfirm={async () => run(() => reviseEstimateAction(jobCardId, estimateId), 'Revision created')}
      />
      {error ? <p className="text-sm text-destructive">{error}</p> : null}
    </div>
  );
...
export function NewLinkButton({ jobCardId, estimateId, customerName }: { jobCardId: string; estimateId: string; customerName: string }) {
  const [link, setLink] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  if (link) return <CustomerLinkPanel link={link} customerName={customerName} onDone={() => setLink(null)} />;
      ...
      <ConfirmAction
        ...
        onConfirm={async () =>
          startTransition(async () => {
            ...
            if (!result.ok || !result.data) return setError(result.error ?? 'Could not create a link.');
...
export function RecordDecisionForm({ jobCardId, estimateId }: { jobCardId: string; estimateId: string }) {
  const router = useRouter();
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-6">
      ...
      <Field label="How did the customer tell you?" htmlFor="method" required error={state.fieldErrors?.method}>
        <NativeSelect id="method" name="method" defaultValue="" required>
      ...
      <TextareaField label="Notes" name="notes" placeholder="e.g. Approved by phone, wants the car by Thursday" />
      <FormError message={state.error} />
      <SubmitButton pending={isPending} variant={decision === 'APPROVED' ? 'default' : 'destructive'} size="lg" className="self-start" pendingLabel="Recording…">
