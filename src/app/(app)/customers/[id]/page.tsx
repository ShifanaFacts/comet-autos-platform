      ...
      {openJobs.length > 0 ? (
        <Panel className="border-primary/25 bg-accent/40">
          <p className="text-sm font-medium">
            {openJobs.length === 1 ? 'A vehicle is in the workshop right now:' : `${openJobs.length} vehicles are in the workshop right now:`}
          </p>
          <ul className="mt-3 flex flex-col gap-2">
            {openJobs.map((job) => (
              <li key={job.id}>
                <Link href={`/job-cards/${job.id}`} className="flex flex-wrap items-center gap-3 text-sm hover:underline">
                  <VehiclePlate plateNumber={job.vehicle.plateNumber} className="px-2 py-0.5 text-xs" />
              ...
              return (
                <Panel key={vehicle.id} className="flex flex-col gap-4">
                  <div className="flex items-start justify-between gap-3">
                    <Link href={`/vehicles/${vehicle.id}`} className="flex min-w-0 flex-col gap-2 hover:underline">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {vehicle.lastMileage !== null ? `${vehicle.lastMileage.toLocaleString('en-AE')} km last recorded` : 'No mileage recorded'}
                  ...
                  <div className="mt-auto flex flex-wrap gap-2 border-t border-border pt-4">
                    ...
                    <LinkButton href={`/appointments/new?vehicle=${vehicle.id}`} size="sm" variant="outline">
                      <CalendarDays />
                      Book
                    </LinkButton>
          ...
          <Panel padding="none">
            <ul className="divide-y divide-border">
              {customer.appointments.map((appointment) => (
                <li key={appointment.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 text-sm sm:px-6">
                  <span className="flex items-center gap-3">
                    <CalendarDays className="size-4 text-muted-foreground" />
                    <span className="font-medium">{formatDateTime(appointment.scheduledAt)}</span>
                    {appointment.vehicle ? <VehiclePlate plateNumber={appointment.vehicle.plateNumber} className="px-2 py-0.5 text-xs" /> : null}
                  </span>
              ...
              <TableBody>
                {jobCards.map((job) => (
                  <TableRow key={job.id} className="relative">
                    <TableCell>
                      <Link href={`/job-cards/${job.id}`} className="font-medium after:absolute after:inset-0 hover:underline">
                        {job.jobNumber}
                      </Link>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="max-w-xs truncate text-muted-foreground">{job.customerComplaint ?? '—'}</TableCell>
                    <TableCell>
                      <JobStatusBadge status={job.status} />
                    </TableCell>
                    <TableCell className="text-muted-foreground">{formatDate(job.openedAt)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              <TableBody>
                {customer.invoices.map((invoice) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="text-muted-foreground">{formatCalendarDate(invoice.issueDate)}</TableCell>
                    <TableCell>
                      <StatusPill tone={invoice.status === 'PAID' ? 'success' : invoice.status === 'VOID' || invoice.status === 'CANCELLED' ? 'neutral' : 'warning'}>
