...
- **apps/web** — Next.js 16 + TypeScript. The entire application: UI,
  routing, Server Actions, Route Handlers, and business logic.
- **PostgreSQL**, accessed exclusively through **Prisma**
  (`apps/web/src/lib/prisma.ts`), called directly from Server
...
- **prisma/** at the repo root — the schema (single source of truth) and
  migrations. Client generates into `apps/web/src/generated/prisma`
...
Business logic lives in plain TypeScript modules under `apps/web/src/lib/`
...
`apps/web/src/lib/auth/authorize.ts` for the enforcement code.
...
- `apps/web/src/proxy.ts` (Next.js's Edge "Proxy", formerly "Middleware")
...
- `DATABASE_URL` is server-only (`apps/web/.env.local`, gitignored), never
  sent to the browser.
